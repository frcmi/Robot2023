...
import frc.robot.HealthMonitor;
...
public class SparkMax extends CANSparkMax {
  ...
  public SparkMax(int canId, MotorType motorType) {
    ...
    System.out.println("INFO: Initializing SparkMAX with ID " + canId);
    if (m_burnFlashCnt > 0) {
      /*Logger.tag("SparkMax")
      .warn(
          ...
          canId,
          m_burnFlashCnt);*/
      System.out.println("INFO: SparkMAX with ID " + canId + " initialized after burning flash, flash count " + m_burnFlashCnt);
    }
    s_monitor.add(this);
  }
  ...
  public SparkMax withInitializer(BiFunction<CANSparkMax, Boolean, Boolean> initialize) {
    ...
    while (initialize.apply(this, true) != true) {
      /**
       * Logger.tag("Spark Max") .warn( "Spark Max ID {}: Failed to initialize, attempt {} of {}",
       * getDeviceId(), setAttemptNumber, kParameterSetAttemptCount);
       */
      System.out.println("WARN: SparkMAX ID " + getDeviceId() + " Failed initialization attempt " + setAttemptNumber + " of " + kParameterSetAttemptCount);
      setAttemptNumber++;

      if (setAttemptNumber >= kParameterSetAttemptCount) {
        // Logger.tag("Spark Max").error("Spark Max ID {}: Failed to initialize!!", getDeviceId());
        System.out.println("ERROR: SparkMAX ID " + getDeviceId() + " Failed to initialize!");
  ...
  public boolean mutate(BiFunction<CANSparkMax, Boolean, Boolean> fcn) {
    ...
    while (result == null || result != true) {
      ...
      System.out.println("WARN: SparkMAX ID " + getDeviceId() + " Failed configuration attempt " + setAttemptNumber + " of " + kParameterSetAttemptCount);
      setAttemptNumber++;

      if (setAttemptNumber >= kParameterSetAttemptCount) {
        /*Logger.tag("Spark Max")
        .error("Spark Max ID {}: Failed to run mutator function!!", getDeviceId());*/
        System.out.println("ERROR: SparkMAX ID " + getDeviceId() + " Failed to configure!");
        configSuccessful = false;
        break;
      }
  ...
  public static void burnFlashInSync() {
    ...
    System.out.println("INFO: SparkMAX Burning Flash Count " + ++m_burnFlashCnt);
    Timer.delay(0.25);
    for (SparkMax max : m_sparkMaxes) {
      // Logger.tag("SparkMax").trace("Burning flash for Can ID {}", max.getDeviceId());
      System.out.println("INFO: SparkMAX Burning flash for CAN ID " + max.getDeviceId());
      max.burnFlash();
    ...
    System.out.println("INFO: SparkMAX Burn Flash Complete");
  }
